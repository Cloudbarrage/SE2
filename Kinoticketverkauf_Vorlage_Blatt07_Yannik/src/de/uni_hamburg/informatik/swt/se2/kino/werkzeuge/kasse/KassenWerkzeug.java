...
public class KassenWerkzeug implements Observer {
	// Anonymous Class to map commands
	...
	// Observable Update Command Map
	private Map<Observable, command> _observableUpdateCommands;
	...
	public KassenWerkzeug(Kino kino) {
		...
		// Observables und Methoden hinzufuegen
		_observableUpdateCommands = new HashMap<Observable, command>();
		addObservables();
	...
	/*
	 * Uebergebe an die Observables this als Observer und ordne ihrem Update ein command zu.
	 */
	public void addObservables() {
		// Add this as Observer for the Observable classes to observe
		_datumAuswaehlWerkzeug.addObserver(this);
		_vorstellungAuswaehlWerkzeug.addObserver(this);

		// Declare Commands
		command datumCommand = new command() {
			public void run(Object arg) {
				setzeTagesplanFuerAusgewaehltesDatum();
			}
		};
		command vorstellungsCommand = new command() {
			public void run(Object arg) {
				setzeAusgewaehlteVorstellung();
			}
		};
		// put Commands into Map
		_observableUpdateCommands.put(_datumAuswaehlWerkzeug, datumCommand);
		_observableUpdateCommands.put(_vorstellungAuswaehlWerkzeug,
				vorstellungsCommand);
	...
	public void update(Observable o, Object arg) {
		_observableUpdateCommands.get(o).run(arg);
	}
}
