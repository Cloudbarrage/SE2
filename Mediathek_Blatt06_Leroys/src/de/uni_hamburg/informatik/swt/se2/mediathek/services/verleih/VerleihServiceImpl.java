        ...
        VerleihService
{
    ...
    /**
     * Erzeugt eine neue HashMap aus dem medienbestand.
     */
            ...
            throws ProtokollierException
    {
        ...
        for (Medium medium : medien)
        {
            if (getVormerkKarteFuer(medium)
                .istErsterVormerker(kunde) || (getVormerkKarteFuer(medium).getAnzahlVormerkungen() == 0))
            {
                Verleihkarte verleihkarte = new Verleihkarte(kunde, medium, ausleihDatum);
                if (getVormerkKarteFuer(medium).istErsterVormerker(kunde))
                {
                    getVormerkKarteFuer(medium).entferneErsteVormerkung();
                }
    ...
    /**
     * Es handelt sich hierbei um eine Hilfsmethode, die mehrfach im VerleihServiceImpl aufgerufen wird.
     * Die Methode gibt die Vormerkkarte fuer ein Medium zurueck .
     */
    private VormerkKarte getVormerkKarteFuer(Medium medium)
    {
    ...
    @Override
    public void merkeMediumVor(Medium medium, Kunde kunde)
    {
        assert medium != null : "Vorbedingung verletzt";
        assert kunde != null : "Vorbedingung verletzt";
        if (istVormerkenMoeglich(medium, kunde))
        {
            getVormerkKarteFuer(medium).fuegeKundeHinzu(kunde);
        }
        informiereUeberAenderung();
    }
    ...
    public boolean istVormerkenMoeglich(Medium medium, Kunde kunde)
    {
        assert medium != null : "Vorbedingung verletzt";
        assert kunde != null : "Vorbedingung verletzt";
        boolean vormerken = (_vormerkKarten.get(medium).istVormerkenMoeglich(kunde));
        if(istVerliehen(medium))
        {
            vormerken = vormerken && (getEntleiherFuer(medium).getKundennummer() != kunde.getKundennummer());
        }
        return vormerken;
    }
    ...
    @Override
    public boolean istErsterVormerker(Medium medium, Kunde kunde)
    {
        assert medium != null : "Vorbedingung verletzt";
        assert kunde != null : "Vorbedingung verletzt";
        return getVormerkKarteFuer(medium).istErsterVormerker(kunde);
    }

    @Override
    public Kunde getVormerkerAnPosition(Medium medium, int x)
    {
        assert x >= 0 : "Vorbedingung verletzt";
        assert x < 3 : "Vorbedingung verletzt";
        assert medium != null : "Vorbedingung verletzt";
        return getVormerkKarteFuer(medium).getVormerkerAnPosition(x);
    }

    @Override
    public int getAnzahlVormerkungen(Medium medium)
    {
        assert medium != null : "Vorbedingung verletzt";
        return getVormerkKarteFuer(medium).getAnzahlVormerkungen();
    }

    @Override
    public boolean istMindestensNmalVorgemerkt(Medium medium, int n)
    {
        assert medium != null : "Vorbedingung verletzt";
        return (getVormerkKarteFuer(medium).getAnzahlVormerkungen() >= n);
    }
}
