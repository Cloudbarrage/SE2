...
public class VormerkMedienauflisterWerkzeug extends ObservableSubWerkzeug
{
    ...
    private void setzeAnzuzeigendeMedien()
    {
        ...
        for (Medium medium : medienListe)
        {
            ...
            switch (_verleihService.getAnzahlVormerkungen(medium))
            {
            case 3:
                vormerker3 = _verleihService.getVormerkerAnPosition(medium, 2);

            case 2:
                vormerker2 = _verleihService.getVormerkerAnPosition(medium, 1);

            case 1:
                vormerker1 = _verleihService.getVormerkerAnPosition(medium, 0);
            }
            //            if (_verleihService.istMindestensNmalVorgemerkt(medium, 1))
            //            {
            //                vormerker1 = _verleihService.getVormerkerAnPosition(
            //                        medium, 0);
            //            }
            //            if (_verleihService.istMindestensNmalVorgemerkt(medium, 2))
            //            {
            //                vormerker2 = _verleihService.getVormerkerAnPosition(
            //                        medium, 1);
            //            }
            //            if (_verleihService.istMindestensNmalVorgemerkt(medium, 3))
            //            {
            //                vormerker3 = _verleihService.getVormerkerAnPosition(
            //                        medium, 2);
            //            }
