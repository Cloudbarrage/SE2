...
public class VerleihServiceImplTest
{
    ...
    public VerleihServiceImplTest()
    {
        ...
        _vormerkkunde2 = new Kunde(new Kundennummer(666989), "paula", "panterina");
    ...
    @Test
    public void testeVormerkVorgaenge()
    {
        Medium medium = _medienListe.get(0);
        _service.merkeMediumVor(medium, _vormerkkunde);
        assertTrue(_service.getAnzahlVormerkungen(medium) == 1);
        assertEquals(_service.getVormerkerAnPosition(medium, 0), _vormerkkunde);
        assertTrue(_service.istErsterVormerker(medium, _vormerkkunde));
        assertTrue(_service.istMindestensNmalVorgemerkt(medium, 1));
        assertFalse(_service.istVormerkenMoeglich(medium, _vormerkkunde));
        assertTrue(_service.istVormerkenMoeglich(medium, _vormerkkunde2));
        _service.merkeMediumVor(medium, _vormerkkunde2);
        assertTrue(_service.getAnzahlVormerkungen(medium) == 2);
        assertEquals(_service.getVormerkerAnPosition(medium, 0), _vormerkkunde);
        assertEquals(_service.getVormerkerAnPosition(medium, 1), _vormerkkunde2);
        assertTrue(_service.istErsterVormerker(medium, _vormerkkunde));
        assertTrue(_service.istMindestensNmalVorgemerkt(medium, 2));
        assertFalse(_service.istVormerkenMoeglich(medium, _vormerkkunde));
        assertFalse(_service.istVormerkenMoeglich(medium, _vormerkkunde2));
        _service.merkeMediumVor(medium, _vormerkkunde);
        assertTrue(_service.getAnzahlVormerkungen(medium) == 2);
        assertEquals(_service.getVormerkerAnPosition(medium, 0), _vormerkkunde);
        assertTrue(_service.istErsterVormerker(medium, _vormerkkunde));
